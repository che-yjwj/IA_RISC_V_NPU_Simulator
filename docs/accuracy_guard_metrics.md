...
- `fetch_metrics.latency_p90`, `fetch_metrics.latency_p99`: 명령 페치 지연의 90/99 분위수. 퍼센트는 2자리까지 반올림된다.
  - 분위수 계산은 최대 4,096개의 표본을 유지하는 reservoir sampling 기반 근사치이며, 표본 제한은 `FETCH_LATENCY_SAMPLE_LIMIT` 상수로 구성된다.
...
## 골든 JSON 유지 방법
1. `python -m scripts.regenerate_accuracy_golden`으로 자동 갱신하거나,
   수동으로 수행하려면 `python -m src.simulator.cli benchmark --instructions 1 --config workloads/demos/accuracy_guard/configs/baseline.json --output /tmp/accuracy_summary.json`을 실행한다.
2. 수동 갱신 시 `accuracy_guard.status`가 `ok`인지 확인하고 `/tmp/accuracy_summary.json`에서 필요한 항목을 골든 파일에 반영한다.
