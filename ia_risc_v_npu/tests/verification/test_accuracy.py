...
class ReferenceRISCVEngine:
    ...
    def _execute_r_type(self, operation, rd, rs1, rs2):
        if rd != 0:
            self.registers[rd] = (
                operation(self.registers[rs1], self.registers[rs2]) & 0xFFFFFFFF
            )
        self.pc += 4

    def execute_add(self, rd, rs1, rs2):
        self._execute_r_type(alu.add, rd, rs1, rs2)

    def execute_sub(self, rd, rs1, rs2):
        self._execute_r_type(alu.sub, rd, rs1, rs2)

    def execute_xor(self, rd, rs1, rs2):
        self._execute_r_type(alu.xor, rd, rs1, rs2)

    def execute_or(self, rd, rs1, rs2):
        self._execute_r_type(alu.or_, rd, rs1, rs2)

    def execute_and(self, rd, rs1, rs2):
        self._execute_r_type(alu.and_, rd, rs1, rs2)
...
class TestInstructionAccuracy:
    ...
    def test_add_instruction(self):
        self._run_single_instruction_test(
            ADD_INSTRUCTION.to_bytes(4, "little"),
            self.ref_engine.execute_add,
            RD_X1,
            RS1_X2,
            RS2_X3,
        )
    ...
    def test_sub_instruction(self):
        self._run_single_instruction_test(
            SUB_INSTRUCTION.to_bytes(4, "little"),
            self.ref_engine.execute_sub,
            RD_X1,
            RS1_X2,
            RS2_X3,
        )
    ...
    def test_xor_instruction(self):
        self._run_single_instruction_test(
            XOR_INSTRUCTION.to_bytes(4, "little"),
            self.ref_engine.execute_xor,
            RD_X1,
            RS1_X2,
            RS2_X3,
        )
    ...
    def test_or_instruction(self):
        self._run_single_instruction_test(
            OR_INSTRUCTION.to_bytes(4, "little"),
            self.ref_engine.execute_or,
            RD_X1,
            RS1_X2,
            RS2_X3,
        )
    ...
    def test_and_instruction(self):
        self._run_single_instruction_test(
            AND_INSTRUCTION.to_bytes(4, "little"),
            self.ref_engine.execute_and,
            RD_X1,
            RS1_X2,
            RS2_X3,
        )
